 ...
 * any sprites, depending on its location.
 * <p>
 * Layer 3 is rendered last, over the top of everything else.
 * <p>
 * 
 ...
 * and to the left over the top of other tiles behind it.
 * <p>
 * 
...
public class Map {
	...
	public void render(Graphics g, int offsetX, int offsetY) {
		...
		int maxY = Math.min((offsetY + viewHeight) / zoomHeight,
				tiles[0].length);
		...
		for (int z = 0; z < LAYERS; z++) {
			for (int x = minX; x < maxX; x++) {
				for (int y = minY; y < maxY; y++) {
					if (tiles[x][y][z] != null)
						tiles[x][y][z].render(g, x * zoomWidth - offsetX, y
								* zoomHeight - offsetY);
	...
	public void render(Graphics g, Point origin, Dimension size) {
		...
		double maxX = Math.min((origin.getX() + size.getWidth()) / zoomWidth,
				tiles.length);

		double minY = Math.max(origin.getY() / zoomHeight, 0);
		double maxY = Math.min((origin.getY() + size.getHeight()) / zoomHeight,
				tiles[0].length);
		...
		for (int z = 0; z < LAYERS; z++) {
			for (int y = (int) minY; y < maxY; y++) {
				for (int x = (int) minX; x < maxX; x++) {
					if (tiles[x][y][z] != null) {
						tiles[x][y][z].render(g, x * zoomWidth + zoomWidth, y
	...
	public void setAllTiles(int[][][] set, GraphicsBank bank) {
		...
		/*
		 * if(set.length == tiles.length && set[0].length == tiles[0].length &&
		 ...
		 * "The int array provided does not match the map dimensions."); }
		 */
	}
