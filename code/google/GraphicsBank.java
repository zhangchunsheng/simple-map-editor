...
public class GraphicsBank {
	...
	public void loadTileset(String from) throws FileNotFoundException,
			IOException {
	 ...
	 * "loadedFrom" tileset.
	 **/
	public void loadTileset(File from) throws FileNotFoundException,
			IOException {
		BufferedReader r;
		String line;
		String tokens[];
		...
		while (true) {
			...
			if (!tileFile.exists()) {
				...
				if (tileFile.exists()) {
					System.err.println("WARNING: file " + tokens[PATH]
							+ " not within the tilemaps working directory");
				} else {
					r.close();
					throw new FileNotFoundException("File " + tokens[PATH]
							+ " referenced on line " + lineCount + " of "
			...
			if (tokens.length > EXTRA) {
				t = new Tile(id, tileFile.toString(), tokens[NAME].trim(),
						tokens[TYPE].trim(), tokens[EXTRA].trim());
			} else {
				t = new Tile(id, tileFile.toString(), tokens[NAME],
						tokens[TYPE]);
			}
			tiles.add(t);
		}
	...
	void saveTileset(File to) throws IOException {
		...
		while (i.hasNext()) {
			...
			String relPath = RelativePath.getRelativePath(
					new File(base.getCanonicalPath()),
					new File(tf.getCanonicalPath()));
			w.print("" + t.getNumber() + ", " + relPath + ", " + t.getName()
					+ ", " + t.getType());
			if (t.getInfo() != null) {
				w.println(", " + t.getInfo());
			} else {
