...
/**
 * map editor. This class may be quite hacky.
 * <p>
...
public class MapEdit implements ActionListener, ChangeListener, KeyListener {
	...
	public MapEdit() {
		...
		mainFrame.setTitle("Map Editor by Judd Modify By Peter");
		...
		/*
		 * outer-most containers actually reserved for docking the toolbars. so
		 * "cp" is actually not the contentpane of the JPanel, but let's ignore
		 * that.
		 */
		...
		String basepath = "../../";
		tilesetOpenBtn = makeBtn("Open", basepath + "icons/opents.gif", "Load Tileset");
		tilesetNewBtn = makeBtn("New", basepath + "icons/newts.gif", "New Tileset");
		tilesetSaveBtn = makeBtn("Save", basepath + "icons/savets.gif", "Save Tileset");
		...
		/*
		 * Continuous repaint every second. This should be eliminated. If it is
		 * even necessary at all, it's only due to bugs.
		 */
		while (true) {
			mapPanel.repaint();
			try {
				...
				System.out
						.println("wow... an interrupted exception. Never seen one before.");
			}
		}
	}
	...
	private void setupToolbars() {
		...
		layerButtons[2] = makeToggleBtn("Layer 3", basepath + "icons/top.gif",
				"Edit the top layer");
		layerButtons[1] = makeToggleBtn("Layer 2", basepath + "icons/mid.gif",
				"Edit the middle layer");
		layerButtons[0] = makeToggleBtn("Layer 1", basepath + "icons/bottom.gif",
		...
		/* Visual buttons */
		gridBtn = makeToggleBtn("Grid", basepath + "icons/grid.gif", "Show/Hide Grid");
		hideBtn = makeToggleBtn("Hide other layers", basepath + "icons/hideoth.gif",
				"Hide other layers");
		/*
		...
		zoomInBtn = makeBtn("Zoom in", basepath + "icons/zoomin.gif", "Zoom in");
		zoomFullBtn = makeBtn("Zoom 100%", basepath + "icons/zoomfull.gif", "Zoom to 100%");
		zoomOutBtn = makeBtn("Zoom out", basepath + "icons/zoomout.gif", "Zoom out");

		/* One-shot map manipulation buttons */
		shiftRightBtn = makeBtn("->", basepath + "icons/shiftRight.gif",
				"Move tiles right");
		shiftLeftBtn = makeBtn("<-", basepath + "icons/shiftLeft.gif", "Move tiles left");
		shiftUpBtn = makeBtn("^", basepath + "icons/shiftUp.gif", "Move tiles up");
		shiftDownBtn = makeBtn("\\/", basepath + "icons/shiftDown.gif", "Move tiles down");
		increaseWidthBtn = makeBtn("<- ->", basepath + "icons/increaseWidth.gif",
				"Increase field width");
		decreaseWidthBtn = makeBtn("-> <-", basepath + "icons/decreaseWidth.gif",
				"Decrease field width");
		increaseHeightBtn = makeBtn("\\/ +", basepath + "icons/increaseHeight.gif",
				"Increase field height");
		decreaseHeightBtn = makeBtn("^^ -", basepath + "icons/decreaseHeight.gif",
				"Decrease field height");

		/* Other map manipulation buttons */
		fillBtn = makeToggleBtn("Flood Fill", basepath + "icons/fill.gif",
				"Flood fill mode");
		undoBtn = makeBtn("Undo", basepath + "icons/undo.gif", "Undo");
		redoBtn = makeBtn("Redo", basepath + "icons/redo.gif", "Redo");

		undoMI.setAccelerator(KeyStroke.getKeyStroke(new Character('Z'),
				Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
		redoMI.setAccelerator(KeyStroke.getKeyStroke(new Character('Y'),
				Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));

		((JPanel) mainFrame.getContentPane()).getInputMap(
		...
		((JPanel) mainFrame.getContentPane()).getInputMap(
				...
				"Redo");
		((JPanel) mainFrame.getContentPane()).getActionMap().put("Undo",
				new undoAction());
		((JPanel) mainFrame.getContentPane()).getActionMap().put("Redo",
				new redoAction());
	...
	/*
	 * This method now handles all buttons rather than using anonymous inner
	 * classes to do it.
	 */
	public void actionPerformed(ActionEvent e) {
		...
		if (source == zoomInBtn) {
			/*
			 * Actually, we could support any max zoom level we like, but let's
			 * be sensible
			 */
			if (zoomLevel < 5) {
				zoomLevel *= 1.2;
			...
			if (openFile == null) {
				...
				actionPerformed(new ActionEvent(saveAsMI, e.getID(),
						e.getActionCommand()));
			} else
				saveFile(openFile);
		 ...
		 *********************/
			...
			System.err
					.println("Unknown source of actionEvent. (The button you just clicked does nothing)");
		}
		/* Multiple buttons may fire the following code */

		if (source == zoomInBtn || source == zoomOutBtn
				|| source == zoomFullBtn || source == effectsResetBtn) {
			scene.setEffect(r.getValue() / 100f, g.getValue() / 100f,
					b.getValue() / 100f, h.getValue() / 360f,
					s.getValue() / 100f, zoomLevel);
	...
	JToggleButton makeToggleBtn(String text, String icon, String tooltip) {
		...
		try {
			newBtn = new JToggleButton(new ImageIcon(getClass().getResource(
					icon)));
		} catch (Exception e) {
	...
	public void newFile() {
		/*
		 * GraphicsBank gfx = new GraphicsBank(); try { //gfx.loadTileset(new
		 * File("gfx/outdoors.dat")); } catch(Exception e) { System.err.println(
		 * "Could not load default graphics bank, using blank one."); gfx = new
		 * GraphicsBank(); }
		 */
	...
	JPanel createColorDialog() {
		...
		/*
		 * dialog.addWindowListener(new WindowListener(){ public void
		 * windowClosing(WindowEvent e) {} public void windowOpened(WindowEvent
		 * e) {} public void windowActivated(WindowEvent e) {} public void
		 * windowClosed(WindowEvent e) {} public void
		 * windowDeactivated(WindowEvent e) { palletteBtn.setSelected(false); }
