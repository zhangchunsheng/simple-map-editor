...
public class MapComponent extends JComponent implements MouseListener,
	...
	public void mapClicked(int x, int y) {
		...
		if (x < map.getWidth() && x >= 0 && y < map.getHeight() && y >= 0) {
			...
			if (mapEdit.getPaintMode() == MapEdit.PAINT_NORMAL) {
				...
				recursiveFlood(x, y, activeLayer,
						map.getTile(x, y, activeLayer),
	...
	/*
	 * Flood fill operation from http://en.wikipedia.org/wiki/Flood_fill The
	 * section used is as follows:
	 * 
	 * Most practical implementations use a loop for the west and east
	 * directions as an optimization to avoid the overhead of stack or queue
	 * management:
	 * 
	 ...
	 * to the east of e no longer matches target-color. 9. Set the color of
	 * nodes between w and e to replacement-color. 10. For each node n between w
	 * and e: 11. If the color of the node to the north of n is target-color,
	 * add that node to Q. If the color of the node to the south of n is
	 * target-color, add that node to Q. 12. Continue looping until Q is
	 * exhausted. 13. Return.
	 */
	public void recursiveFlood(int x, int y, int layer, Tile target,
			Tile replacement) {
		...
		while (right < map.getWidth()
				&& Tile.areEqual(map.getTile(right, y, layer), target)) {
		...
		/*
		 * recursively do this operation above and below. Not so bad as the
		 * other one. the deepest recursion level reachable is the map height.
		 * not great either.
		 */
	...
	public void mousePressed(MouseEvent e) {
		...
		case MouseEvent.BUTTON1:
			...
			break;
		/*
		 * default: btn2Pressed = true; Dimension d = viewport.getSize(); Point
		 * newPoint = new Point((int)(e.getX() - d.getWidth()/2), (int)(e.getY()
		 * - d.getHeight()/2)); viewport.setViewPosition(newPoint);
		 * viewport.repaint(); break;
		 */
