...
public class Tile {
	...
	public boolean equals(Tile t) {
		...
		if (this.number == t.number/*
									 * && this.name.equals(t.name) && this.type
									 * == t.type && this.image == t.image
									 */) {
			return true;
		} else {
			return false;
	...
	void initEffectImage() {
		...
		effectImage = new BufferedImage((int) (imageWidth * effect_zoom),
				(int) (imageHeight * effect_zoom), BufferedImage.TYPE_INT_ARGB);
		Graphics2D g = (Graphics2D) effectImage.getGraphics();
		g.setRenderingHint(RenderingHints.KEY_RENDERING,
				RenderingHints.VALUE_RENDER_QUALITY);
		g.drawImage(image, 0, 0, (int) (imageWidth * effect_zoom),
				(int) (imageHeight * effect_zoom), null, (ImageObserver) null);
		// effectImage.getGraphics().drawImage(image, 0, 0,
		// (ImageObserver)null);
	}
	 ...
	 **/
	void adjustRGBHS(float rScale, float gScale, float bScale, float hueOffset,
			float satScale, float zoom) {
		if (effects_enabled) {

			if (zoom == effect_zoom && rScale == effect_rScale
					&& gScale == effect_gScale && bScale == effect_bScale) {
				return;
			}
			...
			pixelData = raster
					.getPixels(0, 0, zoomWidth, zoomHeight, pixelData);

			for (int i = 0; i < pixelData.length / 4; i++) {

				if (hueOffset != 0 || satScale != 1) {
					hsbVals = Color
							.RGBtoHSB(pixelData[i * 4], pixelData[i * 4 + 1],
									pixelData[i * 4 + 2], hsbVals);
					hsbVals[0] += hueOffset;
					hsbVals[1] *= satScale;
					if (hsbVals[1] > 1) {
						hsbVals[1] = 1;
					}
					rgbVals = new Color(Color.HSBtoRGB(hsbVals[0], hsbVals[1],
							hsbVals[2]));
